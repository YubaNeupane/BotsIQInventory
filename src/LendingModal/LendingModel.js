...
export default function FullScreenDialog({data}) {
  ...
  const sendMail = (data) =>{
    ...
    emailjs.send('default_service', 'template_oEP0naaV', template_params)
  ...
  const handleLending= async()=>{
    ...
    if (!doc.exists) {
      ...
      if(oldData !== undefined){oldData.map(x => data.push(x))}
      ...
      <Dialog fullScreen open={open} onClose={handleClose} TransitionComponent={Transition}>
        <AppBar className={classes.appBar}>
          <Toolbar>
            ...
            <Button size='large'autoFocus color="inherit" onClick={handleLending}>
        ...
        <Container component="main" maxWidth="xs"  >
          <Grid item xs={12}justify="center" style={{marginTop:'15%'}}>
              <InputLabel id="demo-simple-select-helper-label" required>Quantity:</InputLabel>
              <Select 
                required
              ...
              >
                {product.serials.map((serialNumber,index) => <MenuItem key= {serialNumber+index} value={index+1}>{index+1}</MenuItem>)}                
              </Select>
          </Grid>
